...
contract ForkupgradeStakeManagerTest is Test, UpgradeStake_DepositManager_Mainnet {
    ...
    function test_UpgradeStakeManager() public {
        ...
        (bytes memory scheduleBatchPayload, bytes memory executeBatchPayload, bytes32 payloadId) = createPayload(stakeManagerImpl, validatorShareImpl, depositManagerImpl);       
        ...
        assertEq(maticToken.balanceOf(address(depositManagerProxy)), balanceDepositManager/2);
        assertEq(polToken.balanceOf(address(stakeManagerProxy)), balanceDepositManager/2);
    }
}